...
Честный подсчёт токенов контекста и полная формула поиска (70/30 + MMR)
поглощены бэклогом исследований — см. раздел ниже, п. 2 и 9.
...
> **Сделано** (2026-07-22): порт `TaskStore` += `fail_orphaned`/`list_undelivered`;
> sweep `ConversationManager.recover_interrupted()` вызывается из `runtime()` до
> старта планировщика и telegram. PENDING/RUNNING → FAILED «interrupted by
> restart» + пассивная заметка в нарратив (без репорт-прогона); cron-tagged
> проходят через `TaskOutcomeListener` — one-shot получает bounded-ретрай и не
> теряется. Недоставленные результаты редоставляются штатным exactly-once путём
> `_ProcessTerminated` (репорт-прогон или инъекция). Sweep никогда не роняет
> старт. См. [design.md](design.md) «Фоновые задачи».
...
- **Сделано:** п. 1–5 (ошибки/ретраи, usage, реактивная компакция,
  ...
  agent/llm/context/instructions) + п. 7 (startup-recovery осиротевших задач).
- **Сначала дёшево и независимо:** п. 6 (utility-модель), п. 13, п. 14, п. 10 —
  часы-полдня каждый.
- **Затем фундамент:** п. 15 (компакция), п. 11
  (fallback) — надёжность.
