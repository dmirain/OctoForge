...
- **Сначала дёшево и независимо:** п. 6 (utility-модель), п. 4 (идемпотентность),
  п. 5 (prefix-cache) — часы-день каждый, эффект немедленный.
- **Затем фундамент:** п. 1 (ошибки/ретраи) → п. 2 (usage) → п. 3 (реактивная
  компакция + merge) — цепочка зависимостей.
- **Параллельно можно:** п. 7 (startup-recovery), п. 8 (hooks + аудит),
  п. 9 (гибридный поиск) — ни от чего не зависят.
