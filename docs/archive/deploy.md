...
удаление cron-задач, удаление задач и памяти, публикация и удаление инструкций (приватных и
публичных; системные защищены — их пересоздала бы стартовая синхронизация), удаление диалога
целиком (сообщения, задачи, сводки; живой runner останавливается, cron-задачи пользователя
переживают удаление).
...
Шифрованное хранилище для API-ключей пользователей (таблица `secrets`, Fernet). Включается
двумя переменными в `.env`:

```bash
# ключ шифрования — сгенерировать один раз и не терять:
.venv/bin/python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"
OF_SECRETS_KEY=<результат>
# публичный origin, из которого строятся одноразовые ссылки на форму:
OF_PUBLIC_BASE_URL=https://<SITE_DOMAIN>
```

Пустой `OF_SECRETS_KEY` выключает фичу целиком (команда `/secrets` отвечает, что хранилище не
настроено). Пользователь добавляет секреты сам: `/secrets` в Telegram → одноразовая ссылка
(TTL 10 минут) → форма `/secrets.html` вне Basic-гейта (токен ссылки и есть аутентификация).
Значения никогда не проходят через чат и не попадают в контекст LLM: endpoint-запись
декларирует `"auth": {"secret": "<code>", ...}`, подстановка происходит в детерминированном
коде на исходящем HTTP-вызове, значение привязано к одному хосту и вычищается из ответов.

**Ротация ключа не реализована**: смена `OF_SECRETS_KEY` делает старые записи нечитаемыми
(они отвечают как «секрет не найден»), пользователи должны завести их заново. Бэкап ключа —
часть бэкапа `.env`.

## Оверлей системных скилов

`OF_SYSTEM_SKILLS_SOURCE=file:/etc/octoforge/system_skills.json` подменяет/дополняет встроенные
системные скилы при старте, до синхронизации реестра с базой: `append` дописывает к содержимому
(например, русские триггер-фразы для поиска), `content` заменяет его целиком, `tags` — теги.
Файл монтируется в compose (`./docker/system_skills.ru.json:/etc/octoforge/system_skills.json:ro`);
правка файла применяется при следующем старте контейнера, кода не касается.
...
| Секреты | `.env` (LLM-ключ, токен бота, serper, хэш пароля консоли, `OF_SECRETS_KEY`); держи `chmod 600`. Значения не должны содержать `$` — compose его интерполирует. Потеря `OF_SECRETS_KEY` = потеря всех пользовательских секретов |
...
- **Обновление кода:** `docker compose build && docker compose up -d`. Схема доводится на
  старте (`bootstrap_schema`), отдельного шага миграций не нужно. Сборка после правки кода —
  секунды: зависимости живут в отдельном слое, ключуемом только на два `pyproject.toml`
  (Dockerfile выцепляет их список прямо из манифестов и ставит до COPY исходников; сами
  пакеты доставляются в конце через `pip install --no-deps`). Слой с torch пересобирается
  только при изменении зависимостей в `pyproject.toml`.
