...
- **Картинки понимает отдельная модель** (`core/.../vision/`, 29.07): главная
  ...
  **Картинка без подписи — материал**, как пересланное сообщение: пользователь
  ...
  переспрашивает **последнюю** картинку диалога конкретным вопросом; ref'ы
  модели не показываются, тул скрыт (`visible_to`), если зрения нет.
...
- **Пересланные сообщения — материал, а не вопрос**: сообщение с `forward_origin`
  ...
  доступа), пересланное вложение превращается в плейсхолдер, а альбом схлопывается
