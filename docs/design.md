...
- `ProcessStarted(process_id, title, source_client_message_id)` и
  ...
  «сейчас начнётся текст этого ответа» (идёт перед первым токеном форграунда и первым
  событием outbox-доставки; несёт цель реплая) / процесс завершился со статусом
...
- **Процесс** — обработка одного вопроса: свой прогон петли со своей веткой истории,
  ...
  молча. Маркеры `ProcessStarted`/`ProcessCompleted` broadcast'ятся всегда.
...
- **submit**: user-сообщение → нарратив + персист → снимок процессов →
  ...
  **Своя задача помечается явно**: сообщение-источник процесса оборачивается в его ветке
  конвертом `ANSWER_THIS_NOTE_TEMPLATE` («это сообщение ты должен закрыть в этом прогоне»).
  Раньше задача задавалась позицией («твой вопрос — последний в ветке»), и это ломалось
  ровно в очереди: финал форграунда встаёт в нарратив ПОСЛЕ вопроса очередника, ветка
  заканчивается ответом ассистента, модель читает обмен как завершённый и отдаёт пустой
  финал — который путь «осознанного молчания» проглатывал без следа (прод-инцидент
  2026-07-28: «Почему чаек много на море?» + «Какая погода?» → ответ только на первый).
  Пустой финал теперь ещё и логируется (INFO с id задачи) — молчание должно быть заметным.
  Конкурентные процессы не отвечают на чужие вопросы: сообщение-источник другого живого
  answer-процесса оборачивается в ветке конвертом `HANDLED_ELSEWHERE_NOTE_TEMPLATE`
  («этим занят отдельный процесс — не отвечай») и при сборке ветки, и на каждом sync —
  без него каждый процесс видел чужой ещё не отвеченный вопрос и закрывал оба (замер на
  выкатке очереди: очередник «17*23» ответил и на «что ты умеешь», и наоборот). Смена
  покрытия/набора процессов без append в нарратив (START_NEW покрывает сообщение, requeue
  рождает процесс, процесс умирает) ловится wrap-эпохой: `_sync_branch` пересобирает ветку
  не только по росту нарратива, но и по сдвигу `_wrap_epoch` — гейт только по длине
  замораживал устаревший конверт (аудит 2026-07-27). Ранняя доставка готового очередника до конца текущего стрима — осознанно не сделана:
...
- **Mid-run конверт** (`MID_RUN_NOTE_TEMPLATE`, прод-инцидент 26.07): user-сообщение,
  ...
  следующем sync'е, wrap-эпоха гарантирует, что этот sync случится и без роста нарратива).
  Конверт «учти» получает **только форграунд** — уточнение принадлежит прогону, который
  пользователь смотрит; очередники видят такое сообщение как обычную историю (аудит
  2026-07-27: широковещательная нота уводила уточнение во все живые ветки). Осознанные
  компромиссы: (а) отменённый процесс уносит втянутые уточнения без requeue — cancel
  трактуем как «отменить всё, включая уточнения»; (б) в окне «append → решение роутера»
  форграунд может увидеть конверт на сообщении, которое роутер затем отдаст START_NEW —
  лишнее упоминание, не потеря; следующий sync (по эпохе) снимает конверт. Оба — дубль
  вместо молчания: ошибка в безопасную сторону.
- **Завершение процесса**: `Finished` → финал в нарратив + персист (с `task_id` и usage)
  + `mark_done`. **Пустой финал — осознанное молчание** (например, вопрос перехвачен другим
  процессом или пользователь сказал «не надо»): задача завершается DONE и штампуется
  доставленной, но пустое сообщение не попадает ни в нарратив, ни в доставку — пустой
  edit черновика на стороне Telegram вообще падает. `Failed` → `mark_failed`; отмена →
  `mark_cancelled` + гигиена:
...
процессов (`place` определён прямо в промпте: foreground — прогон, чей ответ пользователь
видит стримом; background — очередники и спавны, их результаты приходят отдельными
сообщениями) и правилами: inject — когда сообщение касается текущей работы (комментарии,
...
- **Мост** (`telegram/bridge.py`): `TelegramBridge` на чат: постоянная подписка на события
  ...
  **ответ — реплаем на свой вопрос**: поллер кладёт в `client_message_id` chat-level
  ...
  форграунда и первым событием outbox-доставки — реплай задаётся только при создании
  сообщения, edit его не меняет) и в `Finished`; мост ставит цель на черновик и шлёт
  **первый** чанк с `reply_parameters` (`allow_sending_without_reply` — удалённый вопрос не
  роняет ответ), continuation-чанки — без реплая. None/нечисловой ключ (requeue, старые
  задачи, другой транспорт) — обычное сообщение;
...
- `core/tests/test_conversation_runner.py` — процессы: submit → fg-стрим + ProcessCompleted;
  новый вопрос mid-run → очередник за занятым форграундом (стримит молча, финал
  доставляется целиком после освобождения fg, реплай-ключи в ProcessStarted); INJECT →
  сообщение подтягивается на следующей итерации (pull) с mid-run конвертом у форграунда;
  водяная метка и wrap-эпоха — неувиденное сообщение уходит на повторный
  роутинг, устаревший конверт снимается; «верни задачу X» → START_NEW (свежий процесс
  видит весь нарратив);
