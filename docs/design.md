...
- **`ask_user`**: тул задаёт уточняющий вопрос — вопрос персистится и доставляется
  ...
  закрывает (`asked_user`). Реплика пользователя маршрутизируется в тот же обмен и
  поднимает новый прогон. **Nudge событийный**: новое сообщение при чужом
  AWAITING_USER-обмене старше `NUDGE_AFTER_SECONDS` (5 мин) — отдельное напоминание
  с цитатой собственного вопроса; отдельного таймера нет.
- **Терминация прогона** (`_settle_exchange`): DONE → ANSWERED, отмена → CANCELLED,
  срыв → FAILED; `AWAITING_USER` не перезаписывается (его поставил сам ask). Если
  сообщения обмена пришли после последнего sync'а прогона (`_has_unseen_messages`,
  по watermark'у) — ответ их учесть не мог: обмен возвращается в OPEN и получает
  свежий прогон (`_resume_open_exchange`). Это заменило requeue-эвристику:
  решает durable-состояние, а не пере-submit сообщений.
