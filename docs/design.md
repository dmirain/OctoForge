...
- **Разбор боевого диалога 29.07 — три дефекта зрения и один диалоговый**
  ...
  - **Альбом больше не теряет страницы.** Схлопывание по `media_group_id` было
    ...
    держится тем, что следующее сообщение пользователя сначала выталкивает
    незакрытый альбом (`_flush_albums`) и ждёт уже идущую отправку (лок на
    пользователя: таймер мог сработать за секунду до того, как пользователь
    начал печатать, а описание идёт секунды — иначе вопрос обгонял бы
    картинки, про которые он); `/cancel` исключён — стоп не должен
    стоять в очереди за пачкой медленных вызовов зрения. Страница, которую не
    удалось описать, сохраняет своё место (`(не удалось распознать…)`) и своё
    вложение — `image_look` может вернуться к файлу.
