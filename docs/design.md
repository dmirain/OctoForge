...
- **submit**: user-сообщение → нарратив + персист → снимок процессов →
  ...
  выкатке очереди: очередник «17*23» ответил и на «что ты умеешь», и наоборот). Известная
  щель: процесс requeue-сообщения рождается без append в нарратив, и чужой sync заметит
  его источник только со следующим изменением нарратива — снова «дубль вместо молчания».
