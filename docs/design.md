...
- **submit**: user-сообщение → нарратив + персист → снимок процессов →
  ...
  «верни задачу X» — тоже START_NEW, свежий процесс видит весь нарратив),
...
1. **Сначала поиск.** На любой запрос сложнее small talk (или вопроса, на который
   отвечает сам диалог) первый шаг — `instruction_search` запросом «интент + сущность»
   (`remind reminder`, `report user-data`, `call-api weather`), плюс `memory_search`,
   когда ответ может зависеть от пользователя лично (без query — весь список памяти).
   Оба поиска локальные и дешёвые, вызываются в одном ходу, без анонсов и разрешений:
   лишний поиск стоит меньше пропущенной инструкции. Триггер распространяется и на
...
у `memory_search` — «вызов без query показывает всю память», у `http_request` — «путь
отступления, не по умолчанию: сначала поищи endpoint, не исследуй API перебором»,
у `web_search` — «только публичные факты; своё — в инструкциях и памяти».
...
- **Поиск**: `search(user_id, query, limit)` — видимость «свои + глобальные», подстрока
  ...
  — это **запрос каталога**: те же `limit` самых свежих видимых записей без фильтра
  (раньше — пустой список без запроса в БД). Причина: агент не может угадать подстроку
  для записи, которой ни разу не видел, поэтому «показать всю (небольшую) память» —
  самый дешёвый способ узнать, что он о пользователе помнит; изоляция при этом
  сохраняется — каталог остаётся в границах «свои + глобальные».
  `get`/`delete` — строго по (owner, key): глобальная
  запись через user-скоуп не читается и не удаляется (`MemoryNotFoundError`).
- **Скилы** — тонкие адаптеры: `memory_store(key, content, tags?)`,
  `memory_search(query?, limit?)` (обязательных аргументов нет: вызов без `query` —
  каталог; лимиты `OF_MEMORY_SEARCH_DEFAULT_LIMIT`/
  `OF_MEMORY_SEARCH_MAX_LIMIT`, сниппет 300 символов в одну строку),
  `memory_delete(key)` (not-found — текстом, не исключением). Параметр `scope`
  **убран из агентских тулов** (принятое решение): мгновенно-глобальная запись любым
  пользователем обходила publish-гейт знаний и позволяла окрашивать ответы всем
  остальным; санкционированный путь к общему факту — knowledge-запись + publish
  админом. Глобальный скоуп в сторе остаётся только на чтение (легаси-записи и
  видимость «свои + глобальные» в поиске). Имена с
...
- **Промпт**: методика — в системном скиле `user_memory` (сохранять через `memory_store`,
  scope=user; global — осторожно; не дублировать инструкции/датасеты). В самом системном
  промпте остаётся мета-правило: `memory_search` вызывается всегда, когда ответ может
  зависеть от пользователя лично, и вызов без query показывает всю память. Принятое
  решение: автоинъекция памяти в контекст (шаг 7 «Порядка работ») пока не делается —
  каталог по требованию дешевле по токенам и не ломает prefix-cache системного
  сообщения. Цена решения честная: гарантии, что модель посмотрит в память, нет,
  есть только правило промпта (замер: `memory_search` вызывается на всех
  «личных» запросах и не вызывается на small talk).
...
действие не может обойти инвариант, который соблюдает агент.
