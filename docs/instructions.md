...
> Целевая модель v3 (согласована 2026-07-21). **Статус: дизайн, реализация — следующими
> итерациями**; список расхождений с текущим кодом — в разделе «Миграция с текущей
> модели». Отменяет прежнюю модель «knowledge/skill/tool в едином хранилище» и
> промежуточную v2 «в схеме один skills_search, тулы активируются поиском».
...
Итого типичный ход: сообщение → роутер (маршрутизация + пре-поиск, см. «Пре-поиск
скилов») → сценарии уже в ветке → исполнение тулзами по сценариям. Если интент не
покрыт пре-поиском, агент вызывает `skills_search` сам.
...
применение тулов — следуй сценариям из контекста, а для интентов, не покрытых
пре-поиском, вызови `skills_search`»; каноническая форма запроса (intent + entity)
...
не в промпте. Нормализация интентов первого касания — в роутерном промпте (см.
«Пре-поиск скилов»).
...
- Роутер тулами не пользуется — но участвует в пре-поиске (см. следующий раздел).

## Пре-поиск скилов до прогона

Чтобы основной LLM стартовал уже с релевантными сценариями в контексте (и не тратил
итерацию на поиск), поиск выполняется **до** прогона, бесплатно относительно
LLM-вызовов:

- `LLMRouter` (он и так вызывается на каждое входящее сообщение) возвращает кроме
  ops-пакета список поисковых запросов `searches: [{intent, entity, text?}]` —
  каноническая форма, возможно пустой («ничего не нужно»). Нормализация интентов
  живёт только в роутерном промпте.
- Актор исполняет эти поиски через `InstructionService.search` и подкладывает
  найденные скилы в ветку процесса при старте — system-заметкой с полными текстами.
- Дедуп и бюджет: не более 3 скилов за раз; не инъектить то, что уже есть в
  хвосте/саммари или было инъектировано ранее в этом процессе.
- Работает одинаково для fg, bg и крон-выстрелов (всё проходит через runner).
- Деградация мягкая: если роутер промахнулся с поиском, агент всё ещё может искать
  сам в прогоне (правило «на каждый интент» действует для непокрытых интентов).
- Метрика пользы: доля прогонов, где основной LLM не вызвал `skills_search`
  (цель — большинство).
