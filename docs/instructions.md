...
Итого типичный ход: сообщение → разбор на интенты → `skills_search` на каждый →
сценарии в контексте → исполнение тулзами по сценариям.
...
применение тулов — на каждый интент сообщения сначала `skills_search`, затем следуй
найденным сценариям»; каноническая форма запроса (intent + entity) с примером
маппинга; «после новой многошаговой задачи сохрани сценарий через `instruction_save`».
Пер-туловые правила (крон vs `task_spawn`, работа с памятью, датасеты, `web_search`,
`history_search`) живут в системных сценариях (раздел выше), не в промпте.
