...
**Статус: ОТМЕНЕНО (решение 2026-07-21; код пока на месте, подлежит удалению).**
...
При реализации удаляется: поле `searches` из контракта и промпта роутера, порт
`PresearchPort`, `InstructionPresearch`, инъекция заметки в ветку процесса в акторе.
...
**до** прогона, бесплатно относительно LLM-вызовов:
