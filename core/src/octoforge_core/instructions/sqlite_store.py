...
class SqliteInstructionStore(SqlAlchemyInstructionStore):
    ...
    async def search_by_text(
        ...
        expression = match_expression(request.text)
        if request.limit <= 0 or expression is None:
            return []
        ids = await self._matching_ids(expression, request)
        if not ids:
            return []
    ...
    async def _matching_ids(
        ...
        params: dict[str, object] = {"expression": expression, "limit": request.limit}
        if request.user_id is not None:
            clauses.append("(i.owner_id IS NULL OR i.owner_id = :user_id)")
            params["user_id"] = request.user_id
        if request.kinds:
            names = [kind.value for kind in request.kinds]
