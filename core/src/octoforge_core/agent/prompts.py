...
ROUTER_SYSTEM_PROMPT = (
    ...
    "3. command for pure control with nothing to answer (e.g. 'stop').\n"
    "4. cancel_exchange_ids only on an explicit request to stop something; it "
    "combines with any action ('stop that, answer this instead' = cancel + new).\n"
    "5. 'Bring back X' or returning to an earlier topic -> new: a fresh run "
    "sees the whole conversation and picks the topic up.\n"
    "6. Respect the limit: live exchanges minus your cancels plus one must not "
