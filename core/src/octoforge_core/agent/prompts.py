...
ROUTER_SYSTEM_PROMPT = (
    ...
    "{processes}\n"
    "place=foreground is the run currently streaming its answer to the user; "
    "place=background processes are queued answers or spawned tasks working "
    "silently — their results arrive as separate messages.\n"
    "Decide what the user message means for these processes and ALWAYS answer with "
    "the route tool.\n"
    "Rules:\n"
    "1. inject when the message bears on the work in progress: comments, "
    "corrections, added details or questions about the current task — the run "
    "is told to account for injected messages in its answer -> ops: [inject]. "
    "When unsure, prefer inject.\n"
    "2. start_new for an independent question or request whose answer stands "
    "alone — even a small or playful one — and for anything clearly unrelated "
    "to every active process -> ops: [start_new].\n"
