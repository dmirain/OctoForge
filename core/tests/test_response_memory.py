...
def store_json(memory: ResponseMemory, document: dict[str, object]) -> str:
    body = json.dumps(document, ensure_ascii=False)
    return memory.store(DocumentDraft(OWNER, SCOPE, SOURCE, body, document)).ref


def draft(body: str, document: object = None, scope: str = SCOPE) -> DocumentDraft:
    return DocumentDraft(OWNER, scope, SOURCE, body, document)


def spill_request(body: str, content_type: str, scope: str = "") -> SpillRequest:
    return SpillRequest(OWNER, body, content_type, SOURCE, False, scope=scope)
...
def test_drop_scope_forgets_one_task_only() -> None:
    ...
    kept = memory.store(draft("still here", scope=OTHER_SCOPE)).ref
...
def test_lru_eviction_keeps_the_budget_and_the_newcomer() -> None:
    ...
    old = memory.store(draft("x" * 2000)).ref
    fresh = memory.store(draft("y" * 2000)).ref
...
async def test_json_passport_gives_the_numbers_to_decide_with() -> None:
    ...
    passport = await memory.park(draft(body, document))
...
async def test_text_passport_carries_a_preview() -> None:
    memory = ResponseMemory()
    passport = await memory.park(draft("<html>" + "word " * 500))
...
async def test_find_answers_positions_windows_and_the_full_count() -> None:
    memory = ResponseMemory()
    item = memory.store(draft(HAYSTACK))
...
async def test_find_paging_and_the_narrowing_hint() -> None:
    memory = ResponseMemory()
    item = memory.store(draft("spot " * 100))
    tool = ResponseFindTool(memory)
...
async def test_find_merges_overlapping_windows() -> None:
    ...
    memory = ResponseMemory()
    item = memory.store(draft("aaa bbb aaa bbb aaa"))
    tool = ResponseFindTool(memory)
    answer = await tool.execute(
...
async def test_find_takes_a_broken_regex_literally() -> None:
    memory = ResponseMemory()
    item = memory.store(draft("price is $10 (net"))
    tool = ResponseFindTool(memory)
...
async def test_a_single_document_goes_to_memory_not_the_database() -> None:
    ...
    memory = ResponseMemory()
    spill = ResponseSpill(ResponseSpillOptions(None, CollectionConfig(), memory))
    body = json.dumps({"title": "issue", "body": "т" * 5000})

    passport = await spill.spill(spill_request(body, "application/json", SCOPE))
...
async def test_unstructured_text_goes_to_memory_verbatim() -> None:
    ...
    passport = await spill.spill(spill_request(page, "text/html", SCOPE))
...
async def test_without_any_tier_the_old_truncation_stays() -> None:
    spill = ResponseSpill(ResponseSpillOptions(None, CollectionConfig()))
    body = json.dumps({"title": "x", "body": "y" * 5000})
    assert await spill.spill(spill_request(body, "application/json")) is None
...
def test_the_wire_limit_is_two_megabytes_unless_raised_consciously() -> None:
    ...
    default = ResponseSpill(ResponseSpillOptions(None, CollectionConfig(), ResponseMemory()))
...
async def test_csv_without_a_database_is_remembered_as_text() -> None:
    ...
    spill = ResponseSpill(ResponseSpillOptions(None, CollectionConfig(), ResponseMemory()))
    lines = ["name;amount"] + [f"row-{i};{i}" for i in range(400)]

    passport = await spill.spill(spill_request("\n".join(lines), "text/csv", SCOPE))
...
async def _sqlite_home() -> tuple[DatabaseDocumentHome, ResponseMemory, AsyncEngine]:
...
async def test_a_parked_document_survives_the_task_that_fetched_it() -> None:
    ...
    passport = await home.park(draft(json.dumps({"body": "т" * 3000}), {"body": "т" * 3000}))
...
async def test_the_database_home_round_trips_text_verbatim() -> None:
    ...
    passport = await home.park(draft(page))
    ref = "resp:" + passport.split("resp:", 1)[1].split("]", 1)[0]
...
async def test_with_a_database_the_spill_parks_documents_there() -> None:
    ...
    spill = ResponseSpill(ResponseSpillOptions(store, CollectionConfig(), memory, home))
    body = json.dumps({"title": "issue", "body": "т" * 5000})
...
async def test_build_response_layer_wires_the_database_home_when_collections_exist() -> None:
    ...
    body = json.dumps({"title": "issue", "body": "т" * 5000})
    passport = await layer.spill.spill(spill_request(body, "application/json", SCOPE))
...
async def test_find_is_literal_not_regex_and_shrugs_off_a_redos_pattern() -> None:
    ...
    memory = ResponseMemory()
    item = memory.store(draft("a" * 5000 + " price: $10"))
    tool = ResponseFindTool(memory)
