...
OctoForge is a self-hosted multi-user agent platform: skills, knowledge and callable HTTP endpoint contracts live in the database and are found by embedding search, background work and cron run through the same dialog machinery. Two surfaces ship: a web chat UI and a Telegram bot.

Project documentation lives in `docs/` — start at `docs/README.md` (concept, architecture, one reference page per aspect, guides). Code conventions are `AGENTS.md`. Read them before non-trivial work; this file is a shorter quick-start covering similar ground.
...
| Deployment | `docker compose up -d` = postgres + app (HTTP + console + bot in one process) + caddy (TLS for `SITE_DOMAIN`); `--profile standalone` runs the bot alone. See `docs/guides/deployment.md` |
...
The dialog is an actor, not a request/response handler. The authoritative doc is `docs/reference/exchanges.md`:
...
- **Forwarded messages are material, not questions**: a Telegram forward arrives as `MessageKind.MATERIAL` with attribution baked into the text; it never opens an obligation or starts a run. Material accumulates in one `COLLECTING` exchange per dialog (durable, no in-memory buffer); a sweep (`agent/collecting.py`, `OF_MATERIAL_QUIET_SECONDS`) nominates settled collections and the actor promotes them, so a burst gets one reaction. A question can adopt the collection through the router; at promotion the router may re-parent the batch into another live exchange (its `cancel_ids` are ignored — forwarded text is untrusted).
...
- `cron/` — `CronScheduler` asyncio loop with CAS lease (`lease_ttl`), coalescing missed fires; a fire calls `ConversationManager.wake` → a background process. See `docs/reference/cron.md`.
...
- `context/` — dialog narrative compaction: a rolling summary (`dialog_summaries` table, via `LlmContextCompactor`) plus a verbatim hot tail, triggered by char/token thresholds or reactively on `ContextOverflowError`. See `docs/reference/context-compaction.md`.
...
- **Docs update with code**: any logic change is also written into the matching page under `docs/` in the same change — rules in `docs/CONVENTIONS.md`, and `make check` runs `tools/check_docs.py` over every path and link.
...
- **Parallelize once the plan is set**: tests and the matching `docs/` update don't have to wait for the implementation to land. Once a plan fixes the interfaces and behavior, write (or delegate to parallel subagents) the tests, the docs update, and the implementation at the same time instead of serializing them.
