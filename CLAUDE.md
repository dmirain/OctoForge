...
Tool descriptions carry policy, not just mechanics: they are the only guidance always in context (system skills arrive by search), so `instruction_search` presents itself as the first call for any non-trivial request, `memory_search` advertises its query-less catalog mode, and `http_request` / `web_search` explicitly demote themselves ("search for a stored endpoint first", "public facts only, your own knowledge lives in the store"). The system prompt's rules 1–3 say the same thing. Keep the two in sync — a measured behavior fix, not decoration (see the «Системный промпт» section in `docs/design.md`).
...
- `memory/` — key/value memories, `user_id` NULL = global scope (read-only legacy: the agent tools have no scope parameter — shared facts go through knowledge + admin publish), LIKE search over "own + global"; a blank query is the catalog request (newest visible entries, unfiltered) so the agent can read what it remembers instead of guessing a substring — `memory_search` therefore has no required arguments.
